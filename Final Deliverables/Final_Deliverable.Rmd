...
for (i in 1:length(neighborhoods$Name)) {
  new_neighborhood <- block_tibble[index_matrix[, 3], ] %>%
    inner_join(imputed_block, by = c("Block Group", "Census Tract"))
  
  if(nrow(new_neighborhood) > 0) {
    new_neighborhood$Neighborhood <- neighborhoods$Name[2]
...
neighborhood_touches <- neighborhood_touches %>%
  select(-NAME, -name_popup) %>%
  select(Neighborhood, Profile = cluster, everything())

colnames(neighborhood_touches) <- c("Neighborhood",
                                    "Block Group",
                                    "Census Tract",
                                    "Profile",
                                    "Total Population",
